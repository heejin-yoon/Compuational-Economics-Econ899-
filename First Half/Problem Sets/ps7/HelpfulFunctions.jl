...
function NeweyWest(prim::Primitives, m::Array{Float64}, x::Array{Float64})
    lag_max = 4
    Sy = GammaFunc(prim, m, x, 0)

    # loop over lags
    for i = 1:lag_max
        gamma_i = GammaFunc(prim, m, x, i)
...
function GammaFunc(prim::Primitives, m::Array{Float64}, x::Array{Float64}, lag::Int64)
    @unpack H, T = prim
