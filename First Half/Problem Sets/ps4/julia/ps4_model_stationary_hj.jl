...
function Bellman_retired(prim::Primitives, res::Results)
    @unpack r, b, val_func, pol_func = res
    @unpack N, J_retire, a_grid, n_a, β = prim

    for a_index=1:n_a
        c = (1+r)*a_grid[a_index]+b
        val_func[N, a_index, 1] = u_retired(prim, res, c)
    ...
    for j = N-1:-1:J_retire
        ap_index_min = 1
        for a_index=1:n_a
            wealth = (1+r)*a_grid[a_index]+b
            candidate_max = -Inf
            for ap_index=ap_index_min:n_a
                c = wealth-a_grid[ap_index]
                val = u_retired(prim, res, c) + β*val_func[j+1,ap_index,1]
                if val<candidate_max
                    val_func[j, a_index, 1] = candidate_max
                    pol_func[j, a_index, 1] = a_grid[ap_index-1]
...
function Bellman_worker(prim::Primitives, res::Results)
    @unpack w, r, val_func, pol_func, lab_func = res
            ...
            for a_index=1:n_a
                ...
                for ap_index=ap_index_min:n_a
                    ...
                    if val<candidate_max
                        ...
                        pol_func[j, a_index, z_index] = a_grid[ap_index-1]
                        lab_func[j, a_index, z_index] = optimal_labor(prim, res, a_grid[a_index], a_grid[ap_index-1], j, z_index)
                        ap_index_min = ap_index-1
                        break
                    elseif ap_index==n_a
...
function μ_distribution(prim::Primitives, res::Results)
    ...
    μ[:, :, :] = zeros(N, n_a, n_z)                                          # initialize again
    μ[1, 1, :] = Π₀                                                          # j=1, a=0, z=zᴴ or zᴸ

    for j = 1:(N-1)
        for a_index=1:n_a, z_index=1:n_z
            if μ[j, a_index, z_index] == 0                                   # μ cannot evolve anymore
                continue
            end
            ap_choice = pol_func[j, a_index, z_index]
            for ap_index=1:n_a
                if a_grid[ap_index] == ap_choice
                    for zp_index=1:n_z
                        μ[j+1, ap_index, zp_index] += Π[z_index, zp_index]*μ[j, a_index, z_index]
                    end
                end
            end
        end
    ...
    for z_index=1:n_z
        μ[:,:,z_index]= μ[:,:,z_index].*age_weight
    end
    res.μ = μ
end
...
function solve_model(prim::Primitives, res::Results)
    ...
    println("Aggregate Capital:", res.K0)
    println("Aggregate Labor:", res.L0)
    println("Wage:", res.w)
